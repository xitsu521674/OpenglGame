...
public:
	...
	CatmullClarkSubdivider cs;


	// constructor, expects a filepath to a 3D model.
	Model(string const& path, int times, bool gamma = false) : gammaCorrection(gamma)
	{
		this->times = times;
		loadModel(path);
	}
...
private:
	...
	void processNode(aiNode* node, const aiScene* scene)
	{
		...
		for (unsigned int i = 0; i < node->mNumMeshes; i++)
		{
			...
			if (time != 0) {
				...
				cs.Subdivide(mesh, out, times, false);
				meshes.push_back(processMesh(out, scene));
			}
			else {
	...
	void SetVertexBoneDataToDefault(Vertex2& vertex)
	{
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		Material color = loadMaterial(material);
	...
	void SetVertexBoneData(Vertex2& vertex, int boneID, float weight)
	{
		for (int i = 0; i < MAX_BONE_INFLUENCE; ++i)
		{
	...
	void ExtractBoneWeightForVertices(std::vector<Vertex2>& vertices, aiMesh* mesh, const aiScene* scene)
	{
